/**
 * @fileoverview useStudioTreeViewApi – API Reference Hook
 *
...
/**
 * Props for useStudioTreeViewApi hook
 */
interface UseStudioTreeViewApiProps {
  items: TreeViewItem[];
...
/**
 * useStudioTreeViewApi – API Reference Hook
 *
 ...
 */
export const useStudioTreeViewApi = (props: UseStudioTreeViewApiProps) => {
  const {
    items,
    expandedItems,
    selectedItems,
  ...
  const api = useMemo(
    () => ({
      ...
      setIsItemDisabled: (itemId: TreeViewItemId, disabled: boolean) => {
        if (setIsItemDisabled) {
          setIsItemDisabled(itemId, disabled);
        } else {
