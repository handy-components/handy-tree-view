/**
 * @fileoverview HandyTreeView – Custom Tree View Component
 *
 * A fully custom tree view component that provides all the functionality of
 * RichTreeViewPro without external dependencies. Built for handy-tree-view
 * with full control over behavior and features.
 *
 * Key Features:
...
import { HandyTreeItem } from './HandyTreeItem';
import { useHandyTreeViewApi } from './hooks/useHandyTreeViewApi';
 ...
 */
export interface HandyTreeViewApiRef {
  current?: {
 ...
 */
export interface HandyTreeViewProps {
  /** API reference for programmatic control */
  apiRef?: HandyTreeViewApiRef;
  ...
  /** Optional children to render inside the context provider (useful for testing) */
  children?: React.ReactNode;
}

/**
 * HandyTreeView – Custom Tree View Component
 *
 ...
 * @returns HandyTreeView component
 *
 * @example
 * ```tsx
 * <HandyTreeView
...
export const HandyTreeView = forwardRef<HTMLUListElement, HandyTreeViewProps>(
  (props, ref) => {
    renderCount++;
    const renderId = renderCount;
    console.log(`[HandyTreeView] RENDER START #${renderId}`, { 
    ...
    const lazyLoadingConfig: LazyLoadingConfig | undefined = useMemo(() => {
      console.log('[HandyTreeView] useMemo lazyLoadingConfig', { hasDataSource: !!dataSource, hasLazyLoading: !!lazyLoading });
    ...
    console.log('[HandyTreeView] Calling useLazyLoading', { 
    ...
    console.log('[HandyTreeView] useLazyLoading returned', { 
    ...
    const fullItemsMapRef = useRef<Map<TreeViewItemId, TreeViewItem>>(new Map());
    ...
    const applyFiltering = useCallback(
      (itemsToFilter: TreeViewItem[]): TreeViewItem[] => {
        console.log('[HandyTreeView] applyFiltering', { 
    ...
    const buildFullItemsMap = useCallback(
      (itemsToMap: TreeViewItem[], map: Map<TreeViewItemId, TreeViewItem> = new Map()): Map<TreeViewItemId, TreeViewItem> => {
        console.log('[HandyTreeView] buildFullItemsMap', { 
    ...
    // Compute filtered items using useMemo to prevent infinite loops
    // Store full items map in the same useMemo to keep them in sync
    const { filteredItems, fullMap } = useMemo(() => {
      if (!dataSource) {
        if (items.length > 0) {
          // Store full items map before filtering (always store original items, even if filtered out)
          const fullMapResult = buildFullItemsMap(items);
          fullItemsMapRef.current = fullMapResult;
          
          // Apply filtering
          const filtered = applyFiltering(items);
          return { filteredItems: filtered, fullMap: fullMapResult };
        } else {
          // No items provided
          fullItemsMapRef.current = new Map();
          return { filteredItems: [], fullMap: new Map() };
        }
      }
      // With dataSource, fullItemsMapRef is managed elsewhere
      return { filteredItems: [], fullMap: new Map() };
    }, [items, dataSource, applyFiltering, buildFullItemsMap]);

    // Track previous filtered items to prevent unnecessary updates
    const prevFilteredItemsIdsRef = useRef<string>('');
    const lastSetItemsRef = useRef<TreeViewItem[]>([]);
    
    // Update internal items when filtered items change
    useEffect(() => {
      console.log('[HandyTreeView] useEffect items sync', { 
        hasDataSource: !!dataSource, 
      ...
      if (!dataSource) {
        const targetItems = filteredItems.length > 0 ? filteredItems : [];
        
        // Create a stable identifier for filtered items to compare
        const currentItemsIds = targetItems.map(item => getItemId(item)).join(',');
        const prevItemsIds = prevFilteredItemsIdsRef.current;
        
        // Only update if the items actually changed
        if (currentItemsIds !== prevItemsIds || targetItems.length !== lastSetItemsRef.current.length) {
          if (targetItems.length > 0) {
            console.log('[HandyTreeView] Syncing filtered items to internalItems');
            setInternalItems(targetItems);
            lastSetItemsRef.current = targetItems;
          } else {
            // Clear internalItems when filteredItems is empty
            // This handles both initial render (when items are filtered out) and subsequent filtering
            // If we have items but filteredItems is empty, it means filtering removed everything
            if (items.length > 0 || lastSetItemsRef.current.length > 0) {
              console.log('[HandyTreeView] Clearing internalItems (filtered out or no items)');
              setInternalItems([]);
              lastSetItemsRef.current = [];
            }
          }
          
          // Update ref - always update to track current state
          prevFilteredItemsIdsRef.current = currentItemsIds;
        } else if (targetItems.length === 0 && items.length > 0 && internalItems.length > 0) {
          // Handle case where items were filtered out but internalItems wasn't cleared
          // This can happen on initial render when all items are filtered out
          console.log('[HandyTreeView] Force clearing internalItems (all items filtered out)');
          setInternalItems([]);
          lastSetItemsRef.current = [];
          prevFilteredItemsIdsRef.current = '';
        }
      }
    }, [filteredItems, dataSource, getItemId, items.length]);
    ...
    useEffect(() => {
      console.log('[HandyTreeView] useEffect fetch root items', {
        hasDataSource: !!dataSource,
      ...
      if (!dataSource || !lazyLoadingConfig?.enabled) {
        console.log('[HandyTreeView] Skipping fetch - no dataSource or not enabled');
      ...
      if (internalItems.length > 0 || items.length > 0) {
        console.log('[HandyTreeView] Skipping fetch - already have items');
        hasFetchedRootItemsRef.current = false;
        return;
      }
      ...
      if (hasFetchedRootItemsRef.current) {
        console.log('[HandyTreeView] Skipping fetch - already fetched');
        return;
      }
      
      console.log('[HandyTreeView] FETCHING ROOT ITEMS');
      ...
      lazyLoadingDataSource.getTreeItems({ parentId: undefined })
        .then((rootItems) => {
          console.log('[HandyTreeView] Root items fetched', { 
          ...
          if (rootItems && rootItems.length > 0) {
            ...
            fullItemsMapRef.current = fullMap;
            
            // Apply filtering
            const filtered = applyFiltering(rootItems);
            console.log('[HandyTreeView] Setting internalItems', { 
        ...
        .catch((error) => {
          if (!isMounted) return;
          console.error('[HandyTreeView] Error fetching root items:', error);
          hasFetchedRootItemsRef.current = false;
          // Error state will be handled by lazyLoadingState
        });
      
      return () => {
        console.log('[HandyTreeView] Cleanup fetch root items');
        isMounted = false;
      };
    ...
    useEffect(() => {
      console.log('[HandyTreeView] useEffect updateSelectionItems', { 
        internalItemsLength: internalItems.length 
      });
    ...
    useEffect(() => {
      if (autoExpandOnNavigation && focusedItemId !== null) {
        console.log('[HandyTreeView] useEffect autoExpandOnNavigation', { focusedItemId });
        ...
        if (item) {
          ...
          if (hasChildren && !isCurrentlyExpanded) {
            console.log('[HandyTreeView] Auto-expanding item', { focusedItemId });
    ...
    const handleItemClick = useCallback(
      (event: React.MouseEvent, itemId: TreeViewItemId) => {
        ...
        // Call user interaction handler first (handles single/double-click detection)
        // This allows double-click detection to work even when expansion happens
        ...
        // Handle expansion if clicking on content and expansionTrigger is 'content'
        if (expansionTrigger === 'content') {
          ...
          if (item) {
            ...
            if (hasChildren) {
              toggleExpansion(itemId, event);
              return; // Don't handle selection when expanding via content
            }
          }
        }
    ...
    const renderTreeItems = useCallback(
      (itemsToRender: TreeViewItem[], level: number = 0): React.ReactNode => {
        ...
        console.log(`[HandyTreeView] renderTreeItems [${callId}]`, { 
            ...
            >
              {isItemExpandedState && hasChildren && itemChildren && (
                <Box component="ul" role="group" sx={{ pl: 2, m: 0, listStyle: 'none' }}>
                  {(() => {
                    console.log(`[HandyTreeView] Rendering children for item ${itemId}`, {
        ...
        console.log(`[HandyTreeView] renderTreeItems [${callId}] completed`, {
          itemsCount: itemsToRender.length,
          level,
    ...
    const itemDataContextValue = useMemo(
      () => ({
        getFullItem: (id: TreeViewItemId) => {
          // Use fullMap from useMemo to ensure we have the latest data
          // Also check fullItemsMapRef as a fallback for cases where fullMap might not be available
          const item = fullMap.get(id);
          if (item) return item;
          // Fallback to ref if fullMap doesn't have it (shouldn't happen, but just in case)
          return fullItemsMapRef.current.get(id);
        },
      }),
      [fullMap] // Update when fullMap changes to ensure we have the latest data
    );
    ...
    const getEmptyStateMessage = useCallback((): string | null => {
      if (internalItems.length === 0) {
        ...
        const hasFiltering = !!filterItems || showHiddenFiles === false;
        // Check if filteredItems is empty but items had content (indicating filtering occurred)
        // Use filteredItems directly from the useMemo result
        const filteredItemsLength = Array.isArray(filteredItems) ? filteredItems.length : 0;
        const itemsWereFiltered = rawItemsCount > 0 && filteredItemsLength === 0 && hasFiltering;
        
        if (itemsWereFiltered) {
          // Items exist but were filtered out - show filtering message
          return noItemsMatchMessage || 'No items match your search.';
        } else if (dataSource && rawItemsCount === 0) {
          // Data source is provided but no items loaded - show data source message
          return noDataSourceMessage || 'No data source available.';
        } else {
          // No items available (no data source, just empty items array)
          return emptyStateMessage || 'No items available';
        }
      }
      return null;
    }, [internalItems.length, items.length, filteredItems, dataSource, emptyStateMessage, noItemsMatchMessage, noDataSourceMessage, filterItems, showHiddenFiles]);
    ...
    console.log(`[HandyTreeView] RENDER END #${renderId}`, { 
      internalItemsLength: internalItems.length,
...
HandyTreeView.displayName = 'HandyTreeView';
