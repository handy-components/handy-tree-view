/**
 * @fileoverview End-to-End Tests for StudioTreeView
 *
...
describe('StudioTreeView E2E Tests', () => {
  ...
  describe('Keyboard Navigation Workflows', () => {
    ...
    it('should complete full keyboard navigation workflow', async () => {
      ...
      await waitFor(() => {
        const documentsItem = screen.getByText('Documents');
        expect(documentsItem).toHaveFocus();
      });
      ...
      await waitFor(() => {
        const nested1Item = screen.getByText('nested1.txt');
        expect(nested1Item).toHaveFocus();
      });
      ...
      await waitFor(() => {
        const file3Item = screen.getByText('file3.txt');
        expect(file3Item).toHaveFocus();
      });
    });
    ...
    it('should handle Page Up/Down navigation workflow', async () => {
      ...
      await waitFor(() => {
        const item10 = screen.getByText('Item 10');
  ...
  describe('Performance Workflows', () => {
    it('should handle large tree rendering and interaction', async () => {
      ...
      renderWithTheme(<StudioTreeView items={largeTree} />);
  ...
  describe('Complete Real-World Workflows', () => {
    ...
    it('should handle complete search and filter workflow', async () => {
      const filterItems = (items: TreeViewItem[]) => {
        return items.filter((item) => item.label.includes('Button'));
      };
    ...
    it('should handle complete API-driven workflow', async () => {
      ...
      await waitFor(() => {
        const componentsItem = screen.getByText('components');
        expect(componentsItem).toHaveFocus();
      });
